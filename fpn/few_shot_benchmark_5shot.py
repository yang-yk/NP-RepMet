...
#root='/home/yyk17/yangyk/Repmet/athome/0213/ep10_5shot_0214'
root='/home/v-yukyan/yangyk/NeurIPS_2020/ImageNet_LOC_5shot'
# ------------------------------------------------------------------------------------------------------
...
def neg_get_ovp_rois(roi_targ, rois_embed, neg_psp_embed = None, low_iou = 0.2, high_iou = 0.3):
    ...
    for GT_idx, gtbb in enumerate(roi_targ):
        ...
        if len(neg_inst_indices)==0:
            ...
            do_nms = True
            neg_nms_ovp_rois = gpu_nms_wrapper(0.3, args.gpu)
            if do_nms:
            ...
            if len(neg_inst_indices)>2:
               ...
               #for i in range(len(neg_inst_indices)):
               #    cos_dist_matrix[i,i:] = 0
               ...
               part_size = np.floor(length/5).astype(np.int)
               import numpy.random as npr
               neg_list=[]
               #for i in range(4):
                   #neg_list.append(npr.choice(neg_inst_indices[i*part_size:(i+1)*part_size],size=1))
               #    neg_list.append(neg_inst_indices[i * part_size])
               #neg_list.append(npr.choice(neg_inst_indices[4 * part_size:], size=1))
               neg_list.append(neg_inst_indices[0])
               neg_list.append(neg_inst_indices[1])
               neg_list = np.array(neg_list)
               #neg_list = np.reshape(neg_list,[5])
               neg_inst_indices = np.array([neg_list[0],neg_list[-1]])


            elif len(neg_inst_indices)<2:
               print(neg_inst_indices)
               #print(neg_inst_indices.shape)
               neg_inst_indices=np.concatenate([neg_inst_indices,neg_inst_indices[0]* np.ones((2-len(neg_inst_indices)))],axis=0).astype(np.int)
               print(neg_inst_indices)

            #length = neg_inst_indices.shape[0]
            #quar_len = np.int(length/4.0)
            #neg_inst_indices = np.array([neg_inst_indices[0],neg_inst_indices[quar_len],neg_inst_indices[2*quar_len],neg_inst_indices[3*quar_len],neg_inst_indices[-1]])
    ...
    if len(bb_indices) > 2:
       ...
       bb_list=[]
       cat_list=[]
       bb_list.append(bb_indices[0])
       bb_list.append(bb_indices[1])
...
#yangyk
def neg_compute_tot_embeds(cat_ids, n_cats, neg_custom_embed, use_kmeans=True):
    #neg_tot_embeds = []
    ...
    n_clusters = config.network.REPS_PER_CLASS
    neg_tot_embeds = [None] * (n_cats)
    for iCls in range(n_cats):  # range(max_cls):
        if iCls in cat_ids:
            samples = np.stack([neg_custom_embed[:, i] for i, c in enumerate(cat_ids) if c == iCls], axis=1)
            if (samples.size > 0) and (samples.shape[1] >= n_clusters):
                if use_kmeans:
                    kmeans = KMeans(n_clusters).fit(samples.T)
                    clst = kmeans.cluster_centers_.T
                ...
                clst = np.concatenate(
                    [samples, 1000 * np.ones((config.network.EMBEDDING_DIM, n_clusters - samples.shape[1]))], axis=1)
        else:
            clst = 1000 * np.ones((config.network.EMBEDDING_DIM, n_clusters))
        neg_tot_embeds[iCls] = clst
...
def gen_reps(roidb,N_inst,epi_cats,train_nImg,model_case,sym,epi_root,epi_num,epi_cats_names,new_cats_to_beginning):
    ...
    for aug_round in range(N_inst):
        ...
        for i_cat, cat in enumerate(epi_cats):
            for nImg in train_nImg[cat]:
                ...
                neg_all_cat_ids_heap += neg_cat_ids_epi
    ...
    neg_cat_ids_all = np.array(neg_all_cat_ids_heap)

    tot_embeds = compute_tot_embeds(cat_ids_all, args.Nway, all_custom_embed, use_kmeans=args.train_clustering == 1)
    #yangyk
    neg_tot_embeds = neg_compute_tot_embeds(neg_cat_ids_all, args.Nway, neg_all_custom_embed,use_kmeans=args.train_clustering == 1)
    ...
    for epi_num in range(start_episode,end_episode):
        ...
        if args.gen_episodes == 0:
            ...
            if config.network.REPS_PER_CLASS < args.Nreps_shot*args.Nshot:
                ...
                #config.network.REPS_PER_CLASS = 10
            print(config.network.REPS_PER_CLASS)
