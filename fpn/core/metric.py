...
class neg_EmbedMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        name = 'neg_EmbedLoss'
        super(neg_EmbedMetric, self).__init__(name)
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_rcnn_names(cfg)
    ...
    def update(self, labels, preds):
        ...
        neg_embed_loss = preds[-1].asnumpy()
        if self.ohem:
            label = preds[-2].asnumpy()
        else:
            if self.e2e:
                label = preds[-2].asnumpy()
            else:
                label = labels[-2].asnumpy()

        # calculate num_inst (average on those kept anchors)
        num_inst = np.sum(label > 0)

        self.sum_metric += np.sum(neg_embed_loss)
        self.num_inst += num_inst

        profile_off(tic, 'neg_EmbedMetric metric')
